...
function wpfp_update_post_users($post_id){
    /** pilli **/
    $current_user_id = wp_get_current_user()->ID;
    if ($current_user_id>0){
        $var = get_post_meta($post_id, 'tuttum_users', true);
        $var[] = $current_user_id;
        update_post_meta($post_id, 'tuttum_users', $var);    
    }        
}

function wpfp_remove_user_from_post_meta($post_id){
    /** pilli **/
    $current_user_id = wp_get_current_user()->ID;
    if ($current_user_id>0){
        $var = get_post_meta($post_id, 'tuttum_users', true);
        if ($var){
            $var = array_diff($var, array($current_user_id));
            update_post_meta($post_id, 'tuttum_users', $var);                
        }        
    }        
...
function wpfp_add_to_usermeta($post_id) {
    ...
    /** pilli **/
    wpfp_update_post_users($post_id);
...
function wpfp_list_most_favorited($limit=5) {
    ...
    $query .= " WHERE post_status='publish' AND meta_key='".WPFP_META_KEY."' AND meta_value > 0 ORDER BY meta_value DESC LIMIT 0, $limit";
    ...
    if ($results) {
        ...
        foreach ($results as $o):
            ...
            echo "<li>";
            echo "<a href='".get_permalink($o->post_id)."' title='". $p->post_title ."'>" . $p->post_title . "</a> <sup style=\"display:none;\" class=\"counter\">($o->meta_value)</sup>";
            echo "</li>";
        endforeach;
        echo "</ul>";
    }
...
function wpfp_do_remove_favorite($post_id) {
    ...
    if (is_user_logged_in()) {
        ...
        /** pilli **/
        wpfp_remove_user_from_post_meta($post_id);        
    }
