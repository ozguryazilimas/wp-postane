...
function twitter_messages($username = '', $num = 1, $list = false, $update = true, $linked  = '#', $hyperlinks = true, $twitter_users = true, $encode_utf8 = false) {
	echo '<div id="twitwrapper">'; //start widget HTML code to disply widget to look like theme
	...
	if ($username == '') {
        ...
        if($update) {				
          ...
          echo sprintf( __('%s', 'twitter-for-wordpress'),' <span class="twitter-timestamp"><br /><abbr title="' . date(__('Y/m/d H:i:s'), $time) . '">' . $h_time . '</abbr></span>' );   //displays the date and timestamp for each tweet posted
		...
		if ($list) echo '</ul>';
                      ...
                      $name = "Houston Web Designers"; // and radio button option has been created for the user to opt in or out to display copyright information
					  
            $url = "http://www.dallasprowebdesigners.com/houston-web-design.html";
