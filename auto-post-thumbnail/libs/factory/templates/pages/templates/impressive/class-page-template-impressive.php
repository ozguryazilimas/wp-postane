...
if ( ! class_exists( 'WBCR\Factory_Templates_101\Impressive' ) ) {
	 ...
	 */
	abstract class Impressive extends \Wbcr_FactoryPages448_AdminPage {
		...
		public function __construct( \Wbcr_Factory449_Plugin $plugin ) {
			...
			$this->title_plugin_action_link = __( 'Settings', 'wbcr_factory_pages_448' );
		...
		protected function warningNotice() {
			/*if( WP_CACHE ) {
				$this->printWarningNotice(__("It seems that a caching/performance plugin is active on this site. Please manually invalidate that plugin's cache after making any changes to the settings below.", 'wbcr_factory_pages_448'));
			}*/
		...
		private function showActionsNotice() {
			$notices = [
				[
					...
					'type'       => 'success',
					'message'    => __( 'The settings have been updated successfully!', 'wbcr_factory_pages_448' ) . ( WP_CACHE ? '<br>' . __( "It seems that a caching/performance plugin is active on this site. Please manually invalidate that plugin's cache after making any changes to the settings below.", 'wbcr_factory_pages_448' ) : '' )
				]
			];
                ...
                <div class="wbcr-factory-control">
					...
					<?php if ( $this->type == 'options' ): ?>
                        <input name="<?= $this->plugin->getPluginName() ?>_save_action"
                               ...
                               value="<?php _e( 'Save', 'wbcr_factory_pages_448' ); ?>">
