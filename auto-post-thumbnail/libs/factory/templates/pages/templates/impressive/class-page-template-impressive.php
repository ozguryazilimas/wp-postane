...
namespace WBCR\Factory_Templates_113;
...
if ( ! class_exists( 'WBCR\Factory_Templates_113\Impressive' ) ) {
	/**
	 * Class Wbcr_FactoryPages462_ImpressiveThemplate
	 */
	abstract class Impressive extends \Wbcr_FactoryPages462_AdminPage {
		...
		public function __construct( \Wbcr_Factory462_Plugin $plugin ) {
			$this->menuIcon = FACTORY_TEMPLATES_113_URL . '/pages/templates/impressive/assets/img/webcraftic-plugin-icon.png';
			//$allow_multisite = apply_filters('wbcr_factory_462_core_admin_allow_multisite', false);
			...
			$this->title_plugin_action_link = __( 'Settings', 'wbcr_factory_pages_462' );
		...
		public function assets( $scripts, $styles ) {
			...
			$this->styles->add( FACTORY_TEMPLATES_113_URL . '/pages/templates/impressive/assets/css/impressive.page.template.css' );
		}
		...
		public function flushCacheAndRulesAction() {
			...
			if ( class_exists( 'WBCR\Factory_Templates_113\Helpers' ) ) {
		...
		protected function warningNotice() {
			/*if( WP_CACHE ) {
				$this->printWarningNotice(__("It seems that a caching/performance plugin is active on this site. Please manually invalidate that plugin's cache after making any changes to the settings below.", 'wbcr_factory_pages_462'));
			}*/
		...
		private function showActionsNotice() {
			$notices = [
				[
					...
					'type'       => 'success',
					'message'    => __( 'The settings have been updated successfully!', 'wbcr_factory_pages_462' ) . ( WP_CACHE ? '<br>' . __( "It seems that a caching/performance plugin is active on this site. Please manually invalidate that plugin's cache after making any changes to the settings below.", 'wbcr_factory_pages_462' ) : '' )
				]
			];
                ...
                <div class="wbcr-factory-control">
					...
					<?php if ( $this->type == 'options' ): ?>
                        <input name="<?php echo $this->plugin->getPluginName() ?>_save_action"
                               ...
                               value="<?php _e( 'Save', 'wbcr_factory_pages_462' ); ?>">
		...
		protected function showOptions() {
			...
			if ( isset( $_POST[ $this->plugin->getPluginName() . '_save_action' ] ) ) {
				...
				if ( ! current_user_can( 'administrator' ) && ! current_user_can( $this->capabilitiy ) ) {
					wp_die( __( 'You do not have permission to edit page.', 'wbcr_factory_pages_462' ) );
				}
