...
class WLCMS_Admin_Dashboard  extends WLCMS_Previewable
{
    ...
    private function remove_metaboxes()
    {
        ...
        if ( $this->is_dashboard_all_hidden() && (
             $this->get_settings('hide_at_a_glance') &&
    ...
    public function compile_welcome_content($key) {
        ...
        $content = sprintf("<div id=\"welcome-panel%1\$d\" class=\"welcome-panel\" style=\"display:none\"><div class=\"wlcms-welcome-panel-content\">%2\$s<div class=\"welcome-panel-column-container\">%3\$s</div></div></div>",
            $key, $title, $this->welcome_description);
    ...
    private function make_welcome_panel($key)
    {
        ...
        wp_add_dashboard_widget( 'custom_help_widget'. $key,
            $this->welcome_title,
            array($this, 'welcome_description'), null, array('desc' => $this->get_welcom_description())
        );
    ...
    public function add_own_rss_panel()
    {
        ...
        wp_add_dashboard_widget('wlcms_rss_box',
            !empty($title) ? $title : '&nbsp;' ,
            array($this, 'rss_box')
        );
    }
    ...
    public function rss_box()
    {
        ...
        if ( $error = is_wp_error( $rss ) ) {
            ...
            wlcms_set_css('.index-php .warning-text',
